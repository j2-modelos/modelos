...
function jQueryObserverFactory(){
  jQuery = window.jQuery || window.jQ3;
  ...
  console.log('jquery.Observe is initialized', window, window.Observer);

};

function checkJQueryForObserver() { // tappac as new
  if (typeof jQ3 !== 'undefined') {
    jQueryObserverFactory();
  }else {
    defer( checkJQueryForObserver, 100 );
  }
}
checkJQueryForObserver();
