...
function j2EQueryDadosTarefaDoProcesso(queryTarefa, numProcesso, successCallback){
  if(!(queryTarefa)){
...
function j2EQueryGetProcessoCredentials(numProcesso, successCallback, errorCallback, getOnlyId){
  function _alt(numProcesso, continueCallback){
    ...
    //center : function(url, name, idProcesso, winSize, scrolled, callback, altTitle){
    thisW = j2EOpW.corner( 'https://pje.tjma.jus.br/pje/Processo/ConsultaProcesso/listView.seam', 'consultaProcesso' + guid(), null, { width : 25, height: 25} );
...
PseudoTarefas.prototype.criarTarefa = function(nome, callback, dados){  
  /*function guid() {
    return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
      var r = Math.random() * 16 | 0, v = c === 'x' ? r : (r & 0x3 | 0x8);
      return v.toString(16);
...
function DelayedCall(min, max){
  var _setTimeout = function(h, d, a){
    console.log('delay is ', d)
    setTimeout(h, d, a);
  }
  var _buffer = [];
  var _waiting = false;
