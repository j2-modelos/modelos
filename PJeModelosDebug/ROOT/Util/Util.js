...
function decodeIt(){
  try{
    $('#output').val( decodeURIComponent(escape(window.atob($('#input').val()))) );
  }catch(e){
    $('#output').val( e  );
  }
}
...
function listDuplicadasIt(){
  ...
  if(dup.length > 0)
    $('#outputDuplicadas').val(dup.join('\n') );
  else
    $('#outputDuplicadas').val( "SEM DUPLICATAS" );
}
...
function listContidosNosJaListados(){
  ...
  if(isListed.length > 0)
    $('#outputDuplicadas').val(isListed.join('\n') );
  else
