...
window.activeTimers_ = [];
window.setTimeout = function(func, delay) {
    window.activeTimers++;
    window.activeTimers_.push( {func : func, delay : delay} );
...
(function(){
  ...
  j2.env.j2U.log = function(txt){
    var pp = document.createElement('p'); //mc
    pp.classList.add('pLog');
    pp.innerHTML = txt;
    _windowDocument.getElementById('upLog').appendChild(pp); //mc
    //jQ(window.document.body).find('#upLog')[0].append(jQ('<p/>', {text : txt, class : 'pLog'})[0]);
  };
  j2.env.j2U.progBar = function(val){
    jQ(_windowDocument.getElementById('uBar')).css('width', val +'%'); //mc
    jQ(_windowDocument.getElementById('uBar')).text(val*1+'%'); //mc
  };
  ...
  jQ(css).appendTo(_windowDocument.body); //mc
  jQ(css2).appendTo(_windowDocument.body); //mc
  jQ(html).appendTo(_windowDocument.body); //mc
...
//try {
  ...
  (function () {     
    ...
    window.j2.modelo = { 
      'exp': { /* expediente */
        'win': _window, //mc
        'doc': _window.document, //mc
        'gE': function (id) {
          return _window.document.getElementById(id); //mc
        }
      },/*
      'edt': { editor app 
        'win': myWindow,
      ...
      'par': { /* parente: task or process */
        'win': _window.parent, //mc
        'doc': _window.parent.document, //mc
        'gE': function (id) {
          return _window.parent.document.getElementById(id); //mc
        }
      },
      'sup': { 
    ...
    window.j2.res.MAIN.artfacts = {
      'ref': function () { return (window.j2ModDebug)?(window.j2.ut.isHttps())? 'exkbd8ly9xgnw6s' : '/res/MAIN/artfacts.js' : 'nnll2mref9pfcf1'; },                //mc
    ...
    evBus.once('loaded-ROOT/res/MAIN/artfacts.js', function(){    
      ...
      evBus.once('loaded-'+window.j2.res.XML.classStyles.lib, function(event, xml){
        ...
        window.j2.env.xmls.classStyles = j2.res.XML.classStyles.xmlEncode ? window.j2.mod._._xmlDecode( j2.res.XML.classStyles.xmlEncode.load ) : xml;
    ...
    function crateAutoResize(){
      evBus.on('Editor.onResize', function(event, j2Win){
        ...
        if(j2.modelo.edt.gE('DocEditorCore')) //ps
          _.resizeTo(_.outerWidth, j2.modelo.edt.gE('DocEditorCore').clientHeight+110); // pl*/
    ...
    function processJ2ApiBoot(){ // boot as new
      ...
      forEach(j2.api.extDependency, function(extD){
        ...
        evBus.once('loaded-'+ _lib.lib, function(event, xml){
          ...
          if(_lib.type === 'j2/javascript' || _lib.type === 'j2/styleSheet'){
