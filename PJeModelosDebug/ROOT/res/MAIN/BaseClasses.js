  ...
  (function () { /* inits autoexec */
    ...
    pkg.AlvaraControls = {
      constructor_ : function(args, el){            
        ...
        _.deposito.valor.onkeypress = function(event){
          pkg.monetario.formatation.filtraNumeros(_.deposito.valor, 18, event);
        };
        _.deposito.valor.onkeyup = function(event){
          pkg.monetario.formatation.formataValor(_.deposito.valor, 16, event);          
        };   
                
        _.deposito.valor.onchange = function(event){
          pkg.monetario.formatation.formataValor(_.deposito.valor, 16, event);
        }; 
    ...
    pkg.AutoSave = {
      ...
      constructor_ : function(args, el){            
        ...
        var _ = pkg.AutoSave._ = {
          ctxt : el,
          autoSaveTime : mod.edt.gE('AutoSave-time') || null, // as
          autoSaveTimePar : $(el.par).find('#j2-AutoSave-time')[0], // pc
          id : 'id-' + (j2.env.PJeVars.processo.idTask || j2.env.PJeVars.processo.idProcesso) + '-' + j2.env.modId.id + '-' + j2.env.PJeVars.usuario.login + (function(){ // as2, as2 as new
        ...
        pkg.AutoSave.containers.type = args.containersType || 'default'; // as
        ...
        var weekElapse = 2 * 1000 * 60 * 60 * 24;
      ...
      setEvents : function(args){  
        ...
        /*evBus.on('onFinishEdition', function(event){ //as new
      ...
      initialize : function(args, el){
        ...
        if( pkg.AutoSave.storage[ pkg.AutoSave._.id] ){ // as changed
          var msg = '';
        ...
        //if( args.containersType !== 'fullFrameBody') // as 2
        pkg.AutoSave.intervalId = setInterval(pkg.AutoSave.save, args.saveTimeInterval || 15000); //as 2
          
        j2.log('pkg.AutoSave -> initialized'); //as
      },      
      new : function(){
        ...
        j2.log('pkg.AutoSave -> created new');
                
      },
      recover : function(args){ //as
        pkg.AutoSave.current = pkg.AutoSave.storage[ pkg.AutoSave._.id];
        ...
        switch ( pkg.AutoSave.containers.type ){ // as new
          case 'fullFrameBody':
            ...
            evBus.fire('AutoSave.onRecover', pkg.AutoSave.containers.type, $(mod.exp.doc.body)); //as3
            break;
        ...
        pkg.AutoSave.intervalId = setInterval(pkg.AutoSave.save, args.saveTimeInterval || 15000); // as
        
        
        j2.log('pkg.AutoSave -> recovered');  //as
      },
      save : function(){
        ...
        var tx = new Date().toLocaleTimeString('pt-BR', {hour: 'numeric', minute : 'numeric', second : 'numeric'}); // as
        
        if(pkg.AutoSave._.autoSaveTime) //as
          pkg.AutoSave._.autoSaveTime.innerHTML = tx; //as
        if(pkg.AutoSave._.autoSaveTimePar) //pc
          pkg.AutoSave._.autoSaveTimePar.innerHTML = tx; //pc
        
        j2.log('pkg.AutoSave -> saved -> time: ' + tx); //as
      }    
    };
    ...
    pkg.BlocoAssinaturas = {
      constructor_ : function(args, el){
        ...
        var _ = {
          ctxt : el,
          sel : $(el.edt).find('#selectorsignatario')[0],
          p : $(el.exp).find('#assinatura')[0],
        ...
        if(isObject(args)){
          ...
          if(args.proProvimento)
            if(args.proProvimento === 'sim'){
              _.provimento = pkg.BlocoAssinaturas.provimento;
              _.provimento.insert = true;
          ...
          evBus.once('BlocoAssinaturas.afterParseSelectorXML', function(event, definitions){
            ...
            _.p.appendChild(domify(d));
            
            /*atualizar a lista de elemntos no editor.
             */
            pkg.Selector.loadItems(definitions, _.sel);
          });
        }
      },
      provimento : {
    ...
    pkg.Cabecalho = {
      constructor_ : function(args, el){
        ...
        pkg.Cabecalho._ ={
          ...
          th : win.gE('CabecEnd').nextSibling
        };
    ...
    pkg.ContatosPartes = {
      ...
      convertWhatsAppMessage : function(source){ // wa as new
        if(!(source)){
          var _ = filter(j2.mod.clsCnstr.Selector.instances, {id : 'whatsAppMessages'});          
          source = (_.length) ? _[0] :  null ;
        }
        
        if(!(source))
          return '';
        
        return encodeURI( $(source.lnkEl).text() );
      }
    };
    ...
    pkg.DestinatarioExpedienteFrameComunicacao = {
      constructor_ : function(args, el){
       ...
       /*
        if(isObject(args)){
          args.bindSeletorPessoaFormats && 
          args.bindSeletorPessoaElement && 
          mod.exp.gE(args.bindSeletorrPessoaElement) &&
          j2.mod._.styleSetter(mod.exp.gE(args.bindSeletorPessoaElement), args.bindSeletorPessoaFormats);
      ...
      processarNovosDestinatarios : function(_, args, meioComnicacao){
        ...
        forEach(selPessTodas, function(pss){
          forEach(lines, function(lns){
            if(lns.match( new RegExp(pss.nome)) !== null ){
        ...
        if(!newDests.length && !j2ModDebug)
          return;
        else{
          if(j2ModDebug) forEach([0, 1], function (dbPessSelIdx){
            var pp = selPessTodas[dbPessSelIdx];
            pp.idFrmComId = 'idFrmComId' + dbPessSelIdx;
            newDests[newDests.length] = pp;
          });
        }
      ...
      setEvents : function(_, args){  
        ...
        evBus.on('beforeFihishEdition', cb); // lwapac
    ...
    pkg.DJeControls = {
      ...
      activateControls : function(){
        ...
        if(!(pkg.HtmlTools)){
          pkg.ModalDialog.ok('Erro ao ralizar trasncirção. :(');
          return;
        }
        ...
        evBus.once('DJeControls.onBuildDJeControls', function(event, container){    
          ...
          _.addActionDocRef.click(function(){
            ...
            var doc;
            try {
              doc = pkg.ReferenciaDocumento.getByExtern();
            }catch(e){
    ...
    pkg.DocEditorCore = {
      constructor_ : function(args, el){   
        ...
        /* gerencia o dispaor de DocEditorCore.UIReady */
        evBus.on('onPostConstruct', function(){
          evBus.fire('DocEditorCore.onPostConstructDefered');
        });
        evBus.on('DocEditorCore.onPostConstructDefered', function(){ 
          console.error('ASSERTION');
           /* if (j2.modelo.edt.win.$ && j2.modelo.edt.win.$.ui)
              evBus.fire('DocEditorCore.UIReady');
            else
              setTimeout(function(){
                evBus.fire('DocEditorCore.onPostConstructDefered');
              }, 100);*/
        ...
        pkg.DocEditorCore.setEvents(args, el);
      },
      readyFired : false,
      setEvents : function(args, el){
        evBus.on('DocEditorCore.UIReady', function(){      
          
          if( ! (args && args.needsBuildTabs && (args.needsBuildTabs ==='true' || args.needsBuildTabs ==='sim' ) ) )
            return;
          
          var _$ = j2.modelo.edt.win.$;
          var tabs = _$('#DocEditorInnerCoreTabs');
          
          tabs.tabs({
            create : function(event, ui){
              _$(el.edt).find('#DocEditorInnerCoreTabs-addAction').click(function(){
                
                var tabsLength = tabs.find('ul li').length;
                
                var li = _$('<li>', {
                 /* class : 'ui-tabs-active ui-state-active'*/
                });
                var a = _$('<a>', {
                  href : '#DocEditorInnerCore-p' + tabsLength, 
                  text : tabsLength
                }); 
                
                tabs.find('ul li:last').before(li);
                li.append(a);
                
                var edtContainer = _$('<div>', {
                  id : 'DocEditorInnerCore-p' + tabsLength
                });
                                
                
                tabs.append(edtContainer);
                ...
                evBus.fire('DocEditorCore.TabCreated', edtContainer);

                pkg.DocEditorCore.readyFired = true;
                
                evBus.on('DocEditorCore.TabCreatedDeffered', function(){                  
                  evBus.fire('DocEditorCore.TabCreated', edtContainer);
                });
        ...
        evBus.on('onFinishEdition', 800, function(event){
          ...
          var count = 1;
          forEach($(mod.exp.doc).find("div[j2='Pagina']"), function(d){
            if(d.style.visibility==='hidden' || d.style.display==='none'){
              $(d).prev().remove();
              d.remove();      
            }
            else{
      ...
      proceedFinish : false,
      close : function () {
        pkg.ModalDialog.okCancel('Fechoar o editor?', 'Modelos j2 - Confrimação', 'docEditorCore.closeConfirmation', 'docEditorCore.onCancelClose');
      }
    };
    ...
    pkg.Documento = {
      ...
      getTeorExpediente : function(){ // ndlg4 as new
        ...
        var _tx = '';
        _body.find('p', mod.exp.doc.body).each(function(a, b){
          _tx += jQ3(b).text() + '<br/>'; 
        });
    ...
    pkg.ExpedienteVinculado = { // ndlg as new
      constructor_ : function(args, el){        
        if(!(j2.modelo.par.win.opener)){ // ndlg3
          pkg.ModalDialog.ok('1', 'Modelo j2 - Atenção' );
          pkg.ModalDialog.ok('2', 'Modelo j2 - Atenção' );
          pkg.ModalDialog.ok('3', 'Modelo j2 - Atenção' );
          pkg.ModalDialog.ok('4', 'Modelo j2 - Atenção' );
    ...
    pkg.FerramentasProcesso = {
      constructor_ : function(args, el){
        ...
        if(mod.par.win.location.href.indexOf('newTaskId')!==-1){  //pdf changed
      ...
      whatsappAction : function(_, version){ // wa as new // ndlg
    ...
    pkg.MeiosDeEncaminhamento = {
      constructor_ : function(args, el){
        ...
        if(args && args.versaoAR)
          VERSAO_DO_AR = args.versaoAR;
        ...
        var _ar = {
          __createProc : function(){
            var divC = $('<div>');
            ($(el.thisContext.exp).prop('id') === 'j2Exp' ? $(el.thisContext.exp) : $(el.thisContext.exp).find('#j2Exp')).append(divC);
            
            _ar.context = { 
              exp : divC[0],
              edt : $(el.edt).find('#modAddtCtrls')[0] };
                
            pkg.AR.append2( _ar.context, {
              eventName : [
              ...
              sub : function(){ 
                if(args.conteudoARTituloDocumentoPadrao && (args.conteudoARTituloDocumentoPadrao === 'true' || args.conteudoARTituloDocumentoPadrao ==='sim') ){
                  var a = el;
                  return $(el.exp).find('#expTitle').text();
                }
                else{
                  var a = el;
                  return $(el.exp).find('#singleBody.oficio_numero'.jQId()).text();
                }
              }
            }, VERSAO_DO_AR); 
          },
          create : function(){
            if( !(_ar.context) ){
              if(!(pkg.AR)){
                ...
                evBus.once('AR.onLoadLibs', _ar.__createProc); 
              }else
                _ar.__createProc();
              
            }else{ 
              pkg.AR.visibleOn(null, _ar.context);
            }
          }, 
          hide : function(){ 
            if(pkg.AR && _ar.context){
              pkg.AR.visibleOff(_ar.context);
            }          
          }
        };
        
        var OJVisible = function(TF){
          if(j2.env.modId.unidade.config.OJs.centralMandados)
              return;
          var els = {
            selector : $(el.edt).find('#selectorOJsItems')[0],
            linked : $(el.exp).find('#oficialJustica')[0]
          } ;
          ...
          if(TF){
            ...
            j2.mod._.styleSetter(els.selector, 'Hidden', true );
            j2.mod._.styleSetter(els.linked, 'Hidden', true );
          }
          else{
            ...
            j2.mod._.styleSetter(els.selector, 'Hidden' );
            j2.mod._.styleSetter(els.linked, 'Hidden' );
          }
        }; 
        ...
        _.sel.onchange = function(ev){
          switch(_.sel.value){
            case 'Oficio.Meio.centralDeMandados':
              _.comCentralMandados && OJVisible(true);
              _ar.hide();
              break;
            case 'Oficio.Meio.correios':
              _.comCentralMandados && OJVisible(false);
              _ar.create();
              break;
      ...
      action : function(event){               
        ...
        function createStyles(){
          ...
          if(!(dv)){
            ...
            var __SHORT_CUT_MENU_INNER_CONTENT_CODED = "";
            var _e = gE('ShortCutMenu.Popup');
    ...
    pkg.NormalizarFormatacao = {
      expDivNormalizeinstances : [],
      constructor_ : function(args, el){
        
        /* pode haver multiplas instâncias de #normalizeFormtas no expediente 
         * mas um único controle de acionamento da ação*/
        pkg.NormalizarFormatacao.expDivNormalizeinstances.push(
          $(el.exp).find('#normalizeFormtas')[0]
        );
        if(pkg.NormalizarFormatacao.expDivNormalizeinstances.length > 1)
          return;
        ...
        var _ = {
            ctxt : el,
            ctrls :    $(el.edt).find('#normalizeFormatsCtrls')[0],
            command :  $(el.edt).find('#normalizeFormatsButton')[0],
            classes : {},
            method : null
        };
      ...
      setEvents : function(){
        evBus.on('onFinishEdition', function(event){
            ...
            forEach(pkg.NormalizarFormatacao.expDivNormalizeinstances, function(expDiv){
              expDiv.style.border = '';
    ...
    pkg.OficioCorpo = {
      ...
      setEvents : function(_){
        evBus.on('afterChange.selectorOficioCorpo-Corpo-controls-FinalidadesItens', function(ev, value, select, selI, unk, _sel){
          /* check context */
          if(_.ctxt.exp !== _sel.ctxt.exp)
            return;
        ...
        evBus.on('onChange.selectorOficioCorpo-Corpo-controls-FinalidadesItens', function(ev, value, select, selI, _sel){
          /* check context */
          if(_.ctxt.exp !== _sel.ctxt.exp)
            return;
      ...
      setEvents : function(_, args){
        ...
        var cb2 = function(event, closer){     
          ...
          if(! (_checkMeios() ) ){
            ...
            tx += 'Confirma mesmo prosseguir #:B{MESMO COM OS MEIOS DE COMUICAÇÃO DIVERGENTES} entre o expediente gerado e o que você configurou na janela anterior deste PAC?!';
    ...
    pkg.Pagina2toN =  {
      count : 2, // initial state is 2, incremented in each construction. A proxiam página é a do contador
      countInc : function(){
        ...
        var baseTexts = [
          ...
          {meio:'meioComunicItCentralMandados', 
            text : ((_.deOrdemEmMandadoTradicional) ? 
                    '#:B{#:U{ENCAMINHO}} a(o) senhor(a) Oficial(a) de Justiça desta unidade jurisdicional o presente mandado extraídos destes autos de #:B{#{j2.env.PJeVars.processo.classe}}, acima qualificados, para que de posse do mesmo #:B{PROCEDA À' + ((complProced)? ' '+complProced.toUpperCase() + '}:' : '}:')
                   :
          ...
          {meio:'meioComunicItCentralMandados_juiz', 
            text : '#:B{#:U{DETERMINO}} a(o) senhor(a) Oficial(a) de Justiça desta unidade jurisdicional o presente mandado extraídos destes autos de #:B{#{j2.env.PJeVars.processo.classe}}, acima qualificados, para que de posse do mesmo #:B{PROCEDA À' + ((complProced)? ' '+complProced.toUpperCase() + '}:' : '}:')
          }
        ];
    ...
    pkg.SeletorOJ = {
      constructor_ : function(args, el){
        ...
        var _ = {
          ctxt : el,
          select : $(el.edt).find('#selectorOJsItems')[0],
          linked : $(el.exp).find('#oficialJustica')[0],
          linkedText : $(el.exp).find('#oficialJusticaText')[0]
        };
        ...
        evBus.once('SeletorOJ.afterParseSelectorXML', function(event, definitions){
          ...
          /*atualizar a lista de elemntos no editor.
           */
          pkg.Selector.loadItems(definitions, _.select );
        });        
    ...
    pkg.SeletorPessoa = {
        ...
        var parteJuizo = {
            ...
            "meiosContato": {
                "telefone": (function(){
                  ...
                  lg('ASSERTION: necessita da rotina para recuperar os dados de telefone a serem implementados em undiades autorizadas.xml');
                  return [
                    '(99)3523-7592',
                    '(99)9989-6346'
                  ];
      ...
      constructor_ : function(args, el){
        ...
        var afterProcessPessoasTodasRotina = function(event){
          ...
          evBus.once('SeletorPessoa.afterParseSelectorXML', function(event, definitions){
            ...
            /*atualizar a lista de elemntos no editor.
             */
            forEach(pkg.SeletorPessoa.instances, function(inst){ // wa changed
      ...
      setEvents : function(_, args){        //wa
        ...
        _.propagateChanges && evBus.on('afterChange.'+_.selector.id, 100, function(event, selectorValue, selectorObj, selI, appd, _sel){ // ndlg2
    ...
    pkg.Selector = {
      ...
      constructor_ : function(args, el){
        ...
        if(isObject(args)){
          ...
          if(args.newId){
            ...
            if(args.source)
              pkg.Selector.loadSelectSource(args.source, _.select, _, args.newId);
        ...
        /*_.autoHide = function(tf){ // sah as new //ndlg5 removed
      ...
      changeListAction : function(monitr, _){
        ...
        for(var ii=0; ii < monitr.options.length; ii++){
          ...
          evBus.fire("onChangeList." + _.select.id + '.' + val, _.select.value, _.select, _);
      ...
      changeAction : function(_){
        ...
        switch( appd.type ){
          ...
          case 'plainText':
            ...
            break;
          case 'selectorArray':
            arE = selI.itemContent.selectorArray.arElement[0];
            if(arE.data)
              appd.body = j2Conv(j2.mod.builder.parseVars(arE.data.text.trim().trimSpaces()));
        ...
        evBus.fire("onChange." + _.select.id, _.select.value, _.select, selI, _);
      ...
      delAction : function(monitor, select, _){
        ...
        /*var toDel = mod.exp.gE( monitor.value + '_li'); // ndlg2
        if(toDel)
          toDel.remove();
        else
          j2.err('ASSERTION: Running point unexpected');*/
        var _expLis = mod.exp.doc.querySelectorAll('#' + monitor.value + '_li'); // ndlg2
        if(!(_expLis.length))
          j2.err('ASSERTION: Running point unexpected');
        else
          forEach(_expLis, function(nd){
            nd.remove();
          });
      ...
      loadSelectSource : function(sourceSt, select, ob, id){
        ...
        if(selectorDefModdle.selectorDef.groupsDefs)
          if(selectorDefModdle.selectorDef.groupsDefs.group)
            if(selectorDefModdle.selectorDef.groupsDefs.group.length){
              ...
              // slck
              if(pkg.Selector.cookies.get('ultimaFinalidade-' + selct.id))
    ...
    pkg.ReferenciaDocumento = {
      constructor_ : function(args, el){
        ...
        _.activator.onclick = function(event){
          ...
          //pkg.ReferenciaDocumento.openWindowListAutoSearch(_); // rdp //rdp2
        };
        ...
        /*_.windowListAutoSearchOpener.onclick = function(event){
          pkg.ReferenciaDocumento.openWindowList(_);
        };*/
      ...
      setEvents : function(_){        
        evBus.on('beforeFihishEdition', 9999, function(event, closer){          
          ...
          if(mod.sup.openWindowListAutoSearch) // rdp
            mod.sup.openWindowListAutoSearch.win.close(); // rdp
        });
      ...
      insertId : function(_, event){
        ...
        doc.url = window.location.origin + doc.url;
        pkg.ReferenciaDocumento.prepareHTMLElements(doc);
        pkg.ReferenciaDocumento.processInsertion(doc, event);
      },
      prepareHTMLElements : function(doc){
        var imgViewCln = mod.edt.gE('ReferenciaDocumento.View').cloneNode(true);
        ...
        var u = document.createElement('u');
        spn.style.cursor = 'pointer';
        spn.title = 'documento id ' + doc.id;
        spn.setAttribute('contenteditable', false);
        u.innerHTML = 'id ' + doc.id;


        var spnBs = document.createElement('span');
        spnBs.innerHTML = '&nbsp;';

        var oCSrc = 'window.open(\'' + doc.url + '\', ' + doc.id + ' + \'popUpDocumento\', \'width=780, height=740, scrollbars=yes\').focus();';
    ...
    processInsertion : function(doc, event, defInsEl){ 
      ...
      rng.insertNode( doc.html.spn );
      rng.setStartAfter( doc.html.spn );
      rng.insertNode( doc.html.spnBs );
      rng.setStartAfter( doc.html.spnBs );

      if(doc.html.spn.nextSibling === null){
        var tn = document.createTextNode('&nbsp;');
        rng.insertNode( tn );
        rng.setStartAfter( tn );
      }else{
        if ( doc.html.spn.nextSibling.nodeName === '#text' ){
           if(doc.html.spn.nextSibling.nodeValue.lentgh === 0){
             doc.html.spn.nextSibling.innerHTML = '&nbsp;';
           }
        }
        rng.setStartAfter( doc.html.spn.nextSibling );
      }
    },
      findURL : function(doc, _){
        if( _.selectMethod.value === 'docAutos'){
          var urlPattern = '/pje/Painel/painel_usuario/popup/visualizarExpediente.seam?paramIdProcessoDocumento=#{idDoc}&paramIdProcessoDocumentoBin=#{idBin}&idProcessoDoc=#{idDoc}&idBin=#{idBin}&idProcesso=#{idProcesso}&actionMethod=Painel%2Fpainel_usuario%2FPaniel_Usuario_Oficial_Justica%2Flist.xhtml%3AprocessoDocumentoHome.setIdDocumentoDestacar%28linha.idProcessoDocumento%2Cfalse%29';
          urlPattern = urlPattern.split('#{idDoc}'     ).join( _.ids.doc                                     );
          urlPattern = urlPattern.split('#{idBin}'     ).join( _.ids.bin                                     );
        ...
        if(doc.length===0){          
          ...
          var aA = mod.sup.docList.win.document.getElementsByTagName('a');
          doc.url = '#';
          forEach(aA, function(a){
            var att = '#';
            if(a.getAttribute('href') !== '#')
              att = a.getAttribute('href');

            if ( att === '#' || att === null)
              return;

            if(att.indexOf( 'idProcessoDocumento=' + doc.id) > 0 || att.indexOf( 'idProcessoDoc=' + doc.id) > 0){
              if(doc.url === '#')
                doc.url = att;
              return;
            }
      ...
      getIdBasedOnMethod : function (_){
        switch (_.selectMethod.value){
          case 'docAutos':
            ...
            break;
          case "firstId":
            /* testar se algo está já selcionado na lista */
            if ( pkg.ReferenciaDocumento.findURL( mod.sup.docList.win.getSelection().toString().replace(/\D+/g,'') ) ){
                pkg.ModalDialog.ok('A inserção de id foi cancelada. Você mantém selecionado um id na lista de documentos.', 'ReferenciaDocumento j2 - Atenção');
                return 0;
            }

            var _ = mod.sup.docList.gE('processoDocumentoGridList:tb');
            if(_)
              if(_.getElementsByTagName('div')[0])
                return _.getElementsByTagName('div')[0].innerHTML;
            return 'error';
            break;
          case "listSel":
            return mod.sup.docList.win.getSelection().toString();
            break;
          case "docSel":
            return mod.exp.win.getSelection().toString();
            break;
          case "idManual":
            var __ = _.textIdInput.value;
            _.textIdInput.value='';
            return __;
            break;
          default:
