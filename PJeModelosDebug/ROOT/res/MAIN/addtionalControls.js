...
try {
  (function () { /* inits autoexec */
    ...
    var filterU = new window.j2.mod._._filterUnique;
    //var assign = window.j2.mod._._206;
    //var domify = new window.j2.mod._._233;
    //var evBus = window.j2.mod.eventBus.EventBus;
    var parseVar = window.j2.mod._._parseVar; // pl
    ...
    pkg.AddtionalControls = {
      constructor_ : function(){ 
        ...
        //Spkg.AddtionalControls.injectCSS();
      },
      injectCSS : function(){
        var css_ = function(){
          return '<style type="text/css">' + JSON.parse(localStorage.getItem(j2.res.CSS.j2.lib)).content + '</style>';
        };
    ...
    pkg.WhatsAppInputs = {
      constructor_ : function(){
        ...
        if(!(pkg.SeletorPessoa) )
          return;
        if(!(pkg.SeletorPessoa.selected) )
          return;
        
        var pess = pkg.SeletorPessoa.selected;
        
        _.nome.value = pess.nome; 
        _.CPF.value =  pess.CPFCNPJ; 
        if(pess.meiosContato && pess.meiosContato.telefone && pess.meiosContato.telefone.length){
          var _c = 0;
          forEach(pess.meiosContato.telefone, function(tlf){
            switch(_c){
              case 0: _.wa.value = tlf; _.wa.onchange(); break;
              case 1: _.wa2.value = tlf; break;
              default: break;
            }
            _c++;
          });
    ...
    pkg.GeneralDatePicker = {
      ...
      constructor_ : function(args, el){
        ...
        /* contexto is mod.edt jqueries */
        debugger;
        var _$ = mod.edt.win.jQ3 || el.j2Win.win.$; 
    ...
    evBus.once('loaded-'+ window.j2.res.MAIN.addtionalControls.lib, function(){
      //pkg.AddtionalControls.constructor_(); // ndlg
    });
