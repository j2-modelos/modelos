...
try {
  (function () { /* inits autoexec */
    ...
    pkg.AR = {
      ...
      modeloDef : {},
      append : function(container, contentIterator){
        var ARVersion = pkg.AR.modeloDef.modelos[0].versao[0];
      ...
      updateContent : function(selc){
        ...
        var cnt = {
          p : mod.exp.gE('AR.Body.Identifications.ConteudoDescricao#DescricaoConteudoAR'),
          div : mod.exp.gE('AR.Body.Identifications.ConteudoDescricao#DescricaoConteudoAR').parentElement
        };
      ...
      printPostList : function(){
        ...
        evBus.once('postList.onLoadLibs', function(event){
          mod.sup.open('', 'printPostList', 'width=1092, height=630, scrollbars=1');
    ...
    pkg.AREdtControles = {
      constructor_ : function(args, el){
        var _ = {
          num : mod.edt.gE('AR.EdtControles#Numero'),
          tipo : mod.edt.gE('AR.EdtControles#EtiquetaTipo'),
          action : mod.edt.gE('AR.EdtControles#Action'),
          etq : {
            p : mod.exp.gE('AR.Body.CodigoBarrasAR#PARN'),
            sedexLabel : mod.exp.gE('AR.Body.CodigoBarrasAR#PARNSedex'),
            numeroLabel : mod.exp.gE('AR.Body.CodigoBarrasAR#PARNNumero'),
            body : mod.exp.gE('AR.Body.CodigoBarrasAR'),
            barCode : mod.exp.gE('AR.Body.CodigoBarrasAR#PARN.BarCode')
          },
          form : mod.exp.gE('AR.Body.CodigoBarrasAR#QueryForm'),
          TAObjetos : mod.exp.gE('objetos'),
          chancela : mod.exp.gE('AR.Hedaer@Chancela'),
          numLabelFooter : mod.exp.gE('ar.footer.arNumero'),
          isSedex : function() {
        ...
        pkg.AREdtControles.setEvents(args, _);
      },
      setEvents : function(args, el){
        evBus.on('AREdtControles.afterConfirmARNumber', function(event){ 
          pkg.AREdtControles.stampARNum(el);
        }); 
        evBus.on('AREdtControles.onNotConfirmARNumber', function(event){ 
          el.num.value = '';
        }); 
      },
      stampARNum : function(arEls){
        if(pkg.AR.verifyRegister(arEls.arNum())){
