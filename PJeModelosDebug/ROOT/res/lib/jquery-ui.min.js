...
(function(b,a){if(typeof define==="function"&&define.amd){define(["jquery"],a)}else{a(b.jQuery||b.jQ3)}}(this,function(ak){ak.ui=ak.ui||{};var y=ak.ui.version="1.12.1";var I=0;var al=Array.prototype.slice;ak.cleanData=(function(av){return function(aw){var az,ay,aA;for(aA=0;(ay=aw[aA])!=null;aA++){try{az=ak._data(ay,"events");if(az&&az.remove){ak(ay).triggerHandler("remove")}}catch(ax){}}av(aw)}})(ak.cleanData);ak.widget=function(aD,aC,av){var ax,aA,aw;var aB={};var ay=aD.split(".")[0];aD=aD.split(".")[1];var az=ay+"-"+aD;if(!av){av=aC;aC=ak.Widget}if(ak.isArray(av)){av=ak.extend.apply(null,[{}].concat(av))}ak.expr[":"][az.toLowerCase()]=function(aE){return !!ak.data(aE,az)};ak[ay]=ak[ay]||{};ax=ak[ay][aD];aA=ak[ay][aD]=function(aF,aE){if(!this._createWidget){return new aA(aF,aE)}if(arguments.length){this._createWidget(aF,aE)}};ak.extend(aA,ax,{version:av.version,_proto:ak.extend({},av),_childConstructors:[]});aw=new aC();aw.options=ak.widget.extend({},aw.options);ak.each(av,function(aE,aF){if(!ak.isFunction(aF)){aB[aE]=aF;return}aB[aE]=(function(){function aH(){return aC.prototype[aE].apply(this,arguments)}function aG(aI){return aC.prototype[aE].apply(this,aI)}return function(){var aI=this._super;var aK=this._superApply;var aJ;this._super=aH;this._superApply=aG;aJ=aF.apply(this,arguments);this._super=aI;this._superApply=aK;return aJ}})()});aA.prototype=ak.widget.extend(aw,{widgetEventPrefix:ax?(aw.widgetEventPrefix||aD):aD},aB,{constructor:aA,namespace:ay,widgetName:aD,widgetFullName:az});if(ax){ak.each(ax._childConstructors,function(aF,aE){var aG=aE.prototype;ak.widget(aG.namespace+"."+aG.widgetName,aA,aE._proto)});delete ax._childConstructors}else{aC._childConstructors.push(aA)}ak.widget.bridge(aD,aA);return aA};ak.widget.extend=function(ax){var av=al.call(arguments,1);var ay=0;var aw=av.length;var aA;var az;for(;ay<aw;ay++){for(aA in av[ay]){az=av[ay][aA];if(av[ay].hasOwnProperty(aA)&&az!==undefined){if(ak.isPlainObject(az)){ax[aA]=ak.isPlainObject(ax[aA])?ak.widget.extend({},ax[aA],az):ak.widget.extend({},az)}else{ax[aA]=az}}}}return ax};ak.widget.bridge=function(av,aw){var ax=aw.prototype.widgetFullName||av;ak.fn[av]=function(az){var aB=typeof az==="string";var aA=al.call(arguments,1);var ay=this;if(aB){if(!this.length&&az==="instance"){ay=undefined}else{this.each(function(){var aC;var aD=ak.data(this,ax);if(az==="instance"){ay=aD;return false}if(!aD){return ak.error("cannot call methods on "+av+" prior to initialization; attempted to call method '"+az+"'")}if(!ak.isFunction(aD[az])||az.charAt(0)==="_"){return ak.error("no such method '"+az+"' for "+av+" widget instance")}aC=aD[az].apply(aD,aA);if(aC!==aD&&aC!==undefined){ay=aC&&aC.jquery?ay.pushStack(aC.get()):aC;return false}})}}else{if(aA.length){az=ak.widget.extend.apply(null,[az].concat(aA))}this.each(function(){var aC=ak.data(this,ax);if(aC){aC.option(az||{});if(aC._init){aC._init()}}else{ak.data(this,ax,new aw(az,this))}})}return ay}};ak.Widget=function(){};ak.Widget._childConstructors=[];ak.Widget.prototype={widgetName:"widget",widgetEventPrefix:"",defaultElement:"<div>",options:{classes:{},disabled:false,create:null},_createWidget:function(aw,av){av=ak(av||this.defaultElement||this)[0];this.element=ak(av);this.uuid=I++;this.eventNamespace="."+this.widgetName+this.uuid;this.bindings=ak();this.hoverable=ak();this.focusable=ak();this.classesElementLookup={};if(av!==this){ak.data(av,this.widgetFullName,this);this._on(true,this.element,{remove:function(ax){if(ax.target===av){this.destroy()}}});this.document=ak(av.style?av.ownerDocument:av.document||av);this.window=ak(this.document[0].defaultView||this.document[0].parentWindow)}this.options=ak.widget.extend({},this.options,this._getCreateOptions(),aw);this._create();if(this.options.disabled){this._setOptionDisabled(this.options.disabled)}this._trigger("create",null,this._getCreateEventData());this._init()},_getCreateOptions:function(){return{}},_getCreateEventData:ak.noop,_create:ak.noop,_init:ak.noop,destroy:function(){var av=this;this._destroy();ak.each(this.classesElementLookup,function(aw,ax){av._removeClass(ax,aw)});this.element.off(this.eventNamespace).removeData(this.widgetFullName);this.widget().off(this.eventNamespace).removeAttr("aria-disabled");this.bindings.off(this.eventNamespace)},_destroy:ak.noop,widget:function(){return this.element},option:function(az,ay){var aw=az;var ax;var aA;var av;if(arguments.length===0){return ak.widget.extend({},this.options)}if(typeof az==="string"){aw={};ax=az.split(".");az=ax.shift();if(ax.length){aA=aw[az]=ak.widget.extend({},this.options[az]);for(av=0;av<ax.length-1;av++){aA[ax[av]]=aA[ax[av]]||{};aA=aA[ax[av]]}az=ax.pop();if(arguments.length===1){return aA[az]===undefined?null:aA[az]}aA[az]=ay}else{if(arguments.length===1){return this.options[az]===undefined?null:this.options[az]}aw[az]=ay}}this._setOptions(aw);return this},_setOptions:function(aw){var av;for(av in aw){this._setOption(av,aw[av])}return this},_setOption:function(aw,av){if(aw==="classes"){this._setOptionClasses(av)}this.options[aw]=av;if(aw==="disabled"){this._setOptionDisabled(av)}return this},_setOptionClasses:function(ax){var aw,ay,av;for(aw in ax){av=this.classesElementLookup[aw];if(ax[aw]===this.options.classes[aw]||!av||!av.length){continue}ay=ak(av.get());this._removeClass(av,aw);ay.addClass(this._classes({element:ay,keys:aw,classes:ax,add:true}))}},_setOptionDisabled:function(av){this._toggleClass(this.widget(),this.widgetFullName+"-disabled",null,!!av);if(av){this._removeClass(this.hoverable,null,"ui-state-hover");this._removeClass(this.focusable,null,"ui-state-focus")}},enable:function(){return this._setOptions({disabled:false})},disable:function(){return this._setOptions({disabled:true})},_classes:function(aw){var av=[];var ay=this;aw=ak.extend({element:this.element,classes:this.options.classes||{}},aw);function ax(aB,az){var aA,aC;for(aC=0;aC<aB.length;aC++){aA=ay.classesElementLookup[aB[aC]]||ak();if(aw.add){aA=ak(ak.unique(aA.get().concat(aw.element.get())))}else{aA=ak(aA.not(aw.element).get())}ay.classesElementLookup[aB[aC]]=aA;av.push(aB[aC]);if(az&&aw.classes[aB[aC]]){av.push(aw.classes[aB[aC]])}}}this._on(aw.element,{remove:"_untrackClassesElement"});if(aw.keys){ax(aw.keys.match(/\S+/g)||[],true)}if(aw.extra){ax(aw.extra.match(/\S+/g)||[])}return av.join(" ")},_untrackClassesElement:function(av){var aw=this;ak.each(aw.classesElementLookup,function(ay,ax){if(ak.inArray(av.target,ax)!==-1){aw.classesElementLookup[ay]=ak(ax.not(av.target).get())}})},_removeClass:function(av,ax,aw){return this._toggleClass(av,ax,aw,false)},_addClass:function(av,ax,aw){return this._toggleClass(av,ax,aw,true)},_toggleClass:function(az,ay,aw,ax){ax=(typeof ax==="boolean")?ax:aw;var av=(typeof az==="string"||az===null),aA={extra:av?ay:aw,keys:av?az:ay,element:av?this.element:az,add:ax};aA.element.toggleClass(this._classes(aA),ax);return this},_on:function(ay,az,av){var ax;var aw=this;if(typeof ay!=="boolean"){av=az;az=ay;ay=false}if(!av){av=az;az=this.element;ax=this.widget()}else{az=ax=ak(az);this.bindings=this.bindings.add(az)}ak.each(av,function(aA,aB){function aD(){if(!ay&&(aw.options.disabled===true||ak(this).hasClass("ui-state-disabled"))){return}return(typeof aB==="string"?aw[aB]:aB).apply(aw,arguments)}if(typeof aB!=="string"){aD.guid=aB.guid=aB.guid||aD.guid||ak.guid++}var aC=aA.match(/^([\w:-]*)\s*(.*)$/);var aE=aC[1]+aw.eventNamespace;var aF=aC[2];if(aF){ax.on(aE,aF,aD)}else{az.on(aE,aD)}})},_off:function(av,aw){aw=(aw||"").split(" ").join(this.eventNamespace+" ")+this.eventNamespace;av.off(aw).off(aw);this.bindings=ak(this.bindings.not(av).get());this.focusable=ak(this.focusable.not(av).get());this.hoverable=ak(this.hoverable.not(av).get())},_delay:function(ax,ay){function av(){return(typeof ax==="string"?aw[ax]:ax).apply(aw,arguments)}var aw=this;return setTimeout(av,ay||0)},_hoverable:function(av){this.hoverable=this.hoverable.add(av);this._on(av,{mouseenter:function(aw){this._addClass(ak(aw.currentTarget),null,"ui-state-hover")},mouseleave:function(aw){this._removeClass(ak(aw.currentTarget),null,"ui-state-hover")}})},_focusable:function(av){this.focusable=this.focusable.add(av);this._on(av,{focusin:function(aw){this._addClass(ak(aw.currentTarget),null,"ui-state-focus")},focusout:function(aw){this._removeClass(ak(aw.currentTarget),null,"ui-state-focus")}})},_trigger:function(aw,av,aA){var ax,ay;var az=this.options[aw];aA=aA||{};av=ak.Event(av);av.type=(aw===this.widgetEventPrefix?aw:this.widgetEventPrefix+aw).toLowerCase();av.target=this.element[0];ay=av.originalEvent;if(ay){for(ax in ay){if(!(ax in av)){av[ax]=ay[ax]}}}this.element.trigger(av,aA);return !(ak.isFunction(az)&&az.apply(this.element[0],[av].concat(aA))===false||av.isDefaultPrevented())}};ak.each({show:"fadeIn",hide:"fadeOut"},function(av,aw){ak.Widget.prototype["_"+av]=function(az,aA,ax){if(typeof aA==="string"){aA={effect:aA}}var ay;var aB=!aA?av:aA===true||typeof aA==="number"?aw:aA.effect||aw;aA=aA||{};if(typeof aA==="number"){aA={duration:aA}}ay=!ak.isEmptyObject(aA);aA.complete=ax;if(aA.delay){az.delay(aA.delay)}if(ay&&ak.effects&&ak.effects.effect[aB]){az[av](aA)}else{if(aB!==av&&az[aB]){az[aB](aA.duration,aA.easing,ax)}else{az.queue(function(aC){ak(this)[av]();if(ax){ax.call(az[0])}aC()})}}}});var l=ak.widget;(function(){var az,ay=Math.max,av=Math.abs,aE=/left|center|right/,aB=/top|center|bottom/,aG=/[\+\-]\d+(\.[\d]+)?%?/,ax=/^\w+/,aF=/%$/,aC=ak.fn.position;function aw(aI,aJ,aH){return[parseFloat(aI[0])*(aF.test(aI[0])?aJ/100:1),parseFloat(aI[1])*(aF.test(aI[1])?aH/100:1)]}function aA(aH,aI){return parseInt(ak.css(aH,aI),10)||0}function aD(aI){var aH=aI[0];if(aH.nodeType===9){return{width:aI.width(),height:aI.height(),offset:{top:0,left:0}}}if(ak.isWindow(aH)){return{width:aI.width(),height:aI.height(),offset:{top:aI.scrollTop(),left:aI.scrollLeft()}}}if(aH.preventDefault){return{width:0,height:0,offset:{top:aH.pageY,left:aH.pageX}}}return{width:aI.outerWidth(),height:aI.outerHeight(),offset:aI.offset()}}ak.position={scrollbarWidth:function(){if(az!==undefined){return az}var aK,aH,aI=ak("<div style='display:block;position:absolute;width:50px;height:50px;overflow:hidden;'><div style='height:100px;width:auto;'></div></div>"),aJ=aI.children()[0];ak("body").append(aI);aK=aJ.offsetWidth;aI.css("overflow","scroll");aH=aJ.offsetWidth;if(aK===aH){aH=aI[0].clientWidth}aI.remove();return(az=aK-aH)},getScrollInfo:function(aI){var aJ=aI.isWindow||aI.isDocument?"":aI.element.css("overflow-x"),aK=aI.isWindow||aI.isDocument?"":aI.element.css("overflow-y"),aL=aJ==="scroll"||(aJ==="auto"&&aI.width<aI.element[0].scrollWidth),aH=aK==="scroll"||(aK==="auto"&&aI.height<aI.element[0].scrollHeight);return{width:aH?ak.position.scrollbarWidth():0,height:aL?ak.position.scrollbarWidth():0}},getWithinInfo:function(aK){var aJ=ak(aK||window),aH=ak.isWindow(aJ[0]),aI=!!aJ[0]&&aJ[0].nodeType===9,aL=!aH&&!aI;return{element:aJ,isWindow:aH,isDocument:aI,offset:aL?ak(aK).offset():{left:0,top:0},scrollLeft:aJ.scrollLeft(),scrollTop:aJ.scrollTop(),width:aJ.outerWidth(),height:aJ.outerHeight()}}};ak.fn.position=function(aO){if(!aO||!aO.of){return aC.apply(this,arguments)}aO=ak.extend({},aO);var aN,aR,aH,aP,aI,aM,aS=ak(aO.of),aJ=ak.position.getWithinInfo(aO.within),aL=ak.position.getScrollInfo(aJ),aQ=(aO.collision||"flip").split(" "),aK={};aM=aD(aS);if(aS[0].preventDefault){aO.at="left top"}aR=aM.width;aH=aM.height;aP=aM.offset;aI=ak.extend({},aP);ak.each(["my","at"],function(){var aT=(aO[this]||"").split(" "),aU,aV;if(aT.length===1){aT=aE.test(aT[0])?aT.concat(["center"]):aB.test(aT[0])?["center"].concat(aT):["center","center"]}aT[0]=aE.test(aT[0])?aT[0]:"center";aT[1]=aB.test(aT[1])?aT[1]:"center";aU=aG.exec(aT[0]);aV=aG.exec(aT[1]);aK[this]=[aU?aU[0]:0,aV?aV[0]:0];aO[this]=[ax.exec(aT[0])[0],ax.exec(aT[1])[0]]});if(aQ.length===1){aQ[1]=aQ[0]}if(aO.at[0]==="right"){aI.left+=aR}else{if(aO.at[0]==="center"){aI.left+=aR/2}}if(aO.at[1]==="bottom"){aI.top+=aH}else{if(aO.at[1]==="center"){aI.top+=aH/2}}aN=aw(aK.at,aR,aH);aI.left+=aN[0];aI.top+=aN[1];return this.each(function(){var aX,aZ,aV=ak(this),aT=aV.outerWidth(),aW=aV.outerHeight(),aU=aA(this,"marginLeft"),aY=aA(this,"marginTop"),a0=aT+aU+aA(this,"marginRight")+aL.width,a1=aW+aY+aA(this,"marginBottom")+aL.height,a3=ak.extend({},aI),a2=aw(aK.my,aV.outerWidth(),aV.outerHeight());if(aO.my[0]==="right"){a3.left-=aT}else{if(aO.my[0]==="center"){a3.left-=aT/2}}if(aO.my[1]==="bottom"){a3.top-=aW}else{if(aO.my[1]==="center"){a3.top-=aW/2}}a3.left+=a2[0];a3.top+=a2[1];aX={marginLeft:aU,marginTop:aY};ak.each(["left","top"],function(a4,a5){if(ak.ui.position[aQ[a4]]){ak.ui.position[aQ[a4]][a5](a3,{targetWidth:aR,targetHeight:aH,elemWidth:aT,elemHeight:aW,collisionPosition:aX,collisionWidth:a0,collisionHeight:a1,offset:[aN[0]+a2[0],aN[1]+a2[1]],my:aO.my,at:aO.at,within:aJ,elem:aV})}});if(aO.using){aZ=function(a9){var a7=aP.left-a3.left,a4=a7+aR-aT,a8=aP.top-a3.top,a5=a8+aH-aW,a6={target:{element:aS,left:aP.left,top:aP.top,width:aR,height:aH},element:{element:aV,left:a3.left,top:a3.top,width:aT,height:aW},horizontal:a4<0?"left":a7>0?"right":"center",vertical:a5<0?"top":a8>0?"bottom":"middle"};if(aR<aT&&av(a7+a4)<aR){a6.horizontal="center"}if(aH<aW&&av(a8+a5)<aH){a6.vertical="middle"}if(ay(av(a7),av(a4))>ay(av(a8),av(a5))){a6.important="horizontal"}else{a6.important="vertical"}aO.using.call(this,a9,a6)}}aV.offset(ak.extend(a3,{using:aZ}))})};ak.ui.position={fit:{left:function(aI,aJ){var aK=aJ.within,aP=aK.isWindow?aK.scrollLeft:aK.offset.left,aN=aK.width,aH=aI.left-aJ.collisionPosition.marginLeft,aO=aP-aH,aL=aH+aJ.collisionWidth-aN-aP,aM;if(aJ.collisionWidth>aN){if(aO>0&&aL<=0){aM=aI.left+aO+aJ.collisionWidth-aN-aP;aI.left+=aO-aM}else{if(aL>0&&aO<=0){aI.left=aP}else{if(aO>aL){aI.left=aP+aN-aJ.collisionWidth}else{aI.left=aP}}}}else{if(aO>0){aI.left+=aO}else{if(aL>0){aI.left-=aL}else{aI.left=ay(aI.left-aH,aI.left)}}}},top:function(aJ,aK){var aL=aK.within,aO=aL.isWindow?aL.scrollTop:aL.offset.top,aN=aK.within.height,aH=aJ.top-aK.collisionPosition.marginTop,aP=aO-aH,aI=aH+aK.collisionHeight-aN-aO,aM;if(aK.collisionHeight>aN){if(aP>0&&aI<=0){aM=aJ.top+aP+aK.collisionHeight-aN-aO;aJ.top+=aP-aM}else{if(aI>0&&aP<=0){aJ.top=aO}else{if(aP>aI){aJ.top=aO+aN-aK.collisionHeight}else{aJ.top=aO}}}}else{if(aP>0){aJ.top+=aP}else{if(aI>0){aJ.top-=aI}else{aJ.top=ay(aJ.top-aH,aJ.top)}}}}},flip:{left:function(aU,aH){var aI=aH.within,aQ=aI.offset.left+aI.scrollLeft,aN=aI.width,aK=aI.isWindow?aI.scrollLeft:aI.offset.left,aT=aU.left-aH.collisionPosition.marginLeft,aP=aT-aK,aL=aT+aH.collisionWidth-aN-aK,aR=aH.my[0]==="left"?-aH.elemWidth:aH.my[0]==="right"?aH.elemWidth:0,aO=aH.at[0]==="left"?aH.targetWidth:aH.at[0]==="right"?-aH.targetWidth:0,aJ=-2*aH.offset[0],aM,aS;if(aP<0){aM=aU.left+aR+aO+aJ+aH.collisionWidth-aN-aQ;if(aM<0||aM<av(aP)){aU.left+=aR+aO+aJ}}else{if(aL>0){aS=aU.left-aH.collisionPosition.marginLeft+aR+aO+aJ-aK;if(aS>0||av(aS)<aL){aU.left+=aR+aO+aJ}}}},top:function(aH,aI){var aJ=aI.within,aP=aJ.offset.top+aJ.scrollTop,aO=aJ.height,aN=aJ.isWindow?aJ.scrollTop:aJ.offset.top,aU=aH.top-aI.collisionPosition.marginTop,aS=aU-aN,aV=aU+aI.collisionHeight-aO-aN,aR=aI.my[1]==="top",aT=aR?-aI.elemHeight:aI.my[1]==="bottom"?aI.elemHeight:0,aL=aI.at[1]==="top"?aI.targetHeight:aI.at[1]==="bottom"?-aI.targetHeight:0,aK=-2*aI.offset[1],aQ,aM;if(aS<0){aM=aH.top+aT+aL+aK+aI.collisionHeight-aO-aP;if(aM<0||aM<av(aS)){aH.top+=aT+aL+aK}}else{if(aV>0){aQ=aH.top-aI.collisionPosition.marginTop+aT+aL+aK-aN;if(aQ>0||av(aQ)<aV){aH.top+=aT+aL+aK}}}}},flipfit:{left:function(){ak.ui.position.flip.left.apply(this,arguments);ak.ui.position.fit.left.apply(this,arguments)},top:function(){ak.ui.position.flip.top.apply(this,arguments);ak.ui.position.fit.top.apply(this,arguments)}}}})();var ah=ak.ui.position;var q=ak.extend(ak.expr[":"],{data:ak.expr.createPseudo?ak.expr.createPseudo(function(av){return function(aw){return !!ak.data(aw,av)}}):function(ax,av,aw){return !!ak.data(ax,aw[3])}});
/*!
