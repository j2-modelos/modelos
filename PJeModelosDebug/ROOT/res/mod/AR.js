...
try {
  (function () { /* inits autoexec */
    ...
    var __COOKIES_EXPIRE_TIME___ = 1800/(60*60*34); // ard
    ...
    pkg.AR = {
      ...
      vars : { //ard as new
        ...
        destinatario : {
          ...
          bairro : '[BAIRRO]',
          CEP : (function(){
            var _ = j2.env.PJeVars.expediente.destinatario.match(/[0-9]{5}-[0-9]{3,}/);
            if(_)
              return _[0];
            else
              return "99999-999";
          ...
          cidade : (function(){
              ...
              return end.split('CEP')[0].split(',').pop().split(' - ').slice(0, -2).join('-').trim();
          })(),
          UF : (function(){
              var end = j2.env.PJeVars.expediente.destinatario;            
              var _ = end.match(/[0-9]{5}-[0-9]{3,}/);
              ...
              var it = end.split('CEP')[0].split(',').pop().match(/ [A-Z]{2} /);
              if(it)
                  return it[0].trim();
          })(),
      ...
      isVisible : false,
      setVisible : function(tf, context){        
        context.ARisVisible = tf;
        
        if(tf){
          context.page.show();
          context.page.prev().show();
          $(context.edt).find('#AR.EdtControles'.jQId()).show();
        }else
        {
          context.page.hide();
          context.page.prev().hide();
          $(context.edt).find('#AR.EdtControles'.jQId()).hide();
        }
        
      },
      visibleOn : function(trueOrFalse, context){
        if(context){
          pkg.AR.setVisible(true, context);
          return;
        }
      ...
      visibleOff : function(context){
        if(context){
          pkg.AR.setVisible(false, context);
          return;
        }
      ...
      append2 : function(container, contentIterator, version){
        pkg.AR.version = version || '6.0';
        
        var ARVersion = pkg.AR.getVersion(version || '6.0');       // ard
        
        if(pkg.AR.modeloDef.modelos[0].classStyles)
          ARVersion.classStyles = pkg.AR.modeloDef.modelos[0].classStyles;
        
        var pgIdx = pkg.Pagina2toN.count;
            
        j2.mod.builder.build(ARVersion, 'AR', container );
        
        container.ARisVisible = true;
        container.page = $(container.exp).find('#p' + pgIdx);
      ...
      append : function(container, contentIterator, version){
        ...
        var ARVersion = pkg.AR.getVersion(version || '7.0'); //ard
      ...
      treatePosstList : function(){
        ...
        pkg.AR.getDataPostList(); // as new
      },
      getDataPostList : function(){ // as new
        var _d = localStorage.getItem('pkg.AR.localUserUnidadeData');
        ...
        if(!(isObject(_d.usuarios[_usId(true)])))
          _d.usuarios[_usId(true)] = {
            def : {}
          };
        _d.usuarios[_usId(true)].def = (function(){
          var _ = filter(j2.env.xmls.usuarios.usuarios[0].usuario, {id : j2.env.PJeVars.usuario.CPF });          
          return (_.length) ? _[0] :  {} ;
        })();
        
        
        
        if(!(isObject(_d.unidades[ _unId(true) ])))
          _d.unidades[ _unId(true) ] = {
            def : {}
          };
        _d.unidades[_unId(true)].def = j2.env.modId.unidade;
                
        localStorage.setItem('pkg.AR.localUserUnidadeData', JSON.stringify( _d) );
      ...
      setEvents : function(args, el){        
        ...
        /*var cb = function(event, closer){ // ard commented
    ...
    pkg.ARBodyIdentificationsProcessoQualificacao = {
      constructor_ : function(args, el){
        var img = $(el.exp).find('#AR.Body.Identifications.ProcessoQualificacao#codPorcNumImg'.jQId())[0];
        if(img){
...
//                  + '&style=68&type=C128B&width=310&height=30&xres=1&font=3';
          var src = 'https://www.invertexto.com/image.php?scale=2&rotation=0&font=none&font_size=11&text=' 
                  + j2.env.PJeVars.processo.meanNum()
                  + '&thickness=10&code=code128';
    ...
    pkg.AREdtControles = {
      constructor_ : function(args, el){
        ...
        _.action.onclick = function (event) {
          if(pkg.AREdtControles.checkARNum(_.num.value)){
            pkg.ModalDialog.okCancel('Confirmo ser válido o número de etiqueta de AR digitado referente ao serviço ' + ((_.isSedex()) ? 'SEDEX' : 'Carta') + ' dos correios',
                                     'Modelos j2 - Confrimação de Etiqueta de AR', 'AREdtControles.afterConfirmARNumber.' + _.uuid, 'AREdtControles.onNotConfirmARNumber.' + _.uuid);
        ...
        if(args.isAR7 && args.isAR7 === 'true') { // ard as new
          _.PLP.onchange = function(event){
              ...
              pkg.AR.cookies.set('PLP', 'teste'); // newCook
      ...
      setEvents : function(args, el, elCtxt){
        evBus.on('AREdtControles.afterConfirmARNumber.' + el.uuid, function(event, sysEvent){ 
          if(args.isAR7 && args.isAR7 === 'true'){ // ard as new
        ...
        evBus.on('AREdtControles.onNotConfirmARNumber.' + el.uuid, function(event, sysEvent){ 
          el.num.value = '';
        }); 
      ...
      stampARNum : function(arEls, ev, checkArNum, elCtxt){
        if(pkg.AR.verifyRegister(arEls.arNum()) && !(checkArNum)){
          if(ev.shiftKey){
            evBus.once('AREdtControles.repeatARNum.Cancel.' + arEls.uuid, function(event){ 
              arEls.num.value = '';
        ...
        j2.mod._.styleSetter( $(elCtxt.edt).find('#AR.EdtControles#Action-Stamp'.jQId())[0], 'Hidden');
        j2.mod._.styleSetter( $(elCtxt.edt).find('#AR.EdtControles#Action-Refresh'.jQId())[0], 'Hidden', true);
        ...
        arEls.etq.p.setAttribute('onclick', "(" + pkg.AREdtControles.sroQuery + ")('" + arEls.arNum() + "', event)");
      ...
      sroQuery : function(numAr, event){
        ...
        var target = event.relatedTarget || event.currentTarget;
        
        var clnForm = target.nextSibling.cloneNode(true);
