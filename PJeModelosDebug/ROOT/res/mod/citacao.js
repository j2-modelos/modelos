    ...
    pkg.Citacao = {
      ...
      setEvents : function(){
        var cb = function(event, closer){
           if(pkg.Citacao.containers.edt.itemsMonitor().childNodes.length===0){
             ...
             'Citacao.noEditionsConfirmed;beforeFinishEdition.postConfirmation', 'docEditorCore.onCancelClose');
           }
        };
        ...
        evBus.on('onChange.'+pkg.Citacao.containers.edt.meio().id, function(event, value, obSelect){
          pkg.Citacao.formatByExpMeio(value);
        });
        
        evBus.on('Citacao.onSelectItemLinkedExecutionValue', function(event, value, obSelect){
          ...
          }else if(j2.env.modId.modAut.versao === '3.2'){
            tools.show( mod.exp.gE('secao.execucaoInfo') );
          }
        });
                                
        /* trata antes de adicionar item */
        evBus.on('beforeChange.'+pkg.Citacao.containers.edt.itemSelector().id, function(event, selectorObj, selI, toAppd){
          var arElByMeio = filter(selI.itemContent.selectorArray.arElement, {keyEl : pkg.Citacao.containers.edt.meio().value } );
      ...
      formatByExpMeio : function(meio){
        ...
        var makeDadoEPassadoVisible = function(TR){          
          ...
          if(_)
            j2.mod._.styleSetter(_, 'Hidden', TR );
        };
              ...
              forEach(sec.inf.getElementsByTagName('*'), function(el){
                ...
                switch(el.id){
                  ...
                  case 'singleBody.citacao.valorExecucao':
                    ...
                    break;
                  case 'vocativo':
                    (j2.env.modId.modAut.versao.match(/3.2/)) && j2.mod._.styleSetter(el, 'b', REMOVE_STYLE ); // nct
                    break;
                  case 'vocativoPreText':
        ...
        switch(meio){
          ...
          case 'meioComunicItDJe':
            changeTitulo('EDITAL DE CITAÇÃO');
            makeARVisible(false);
            makeSelectorOJVisible(false);
            //placeDestinatarioControl('sup');
            //makeDadoEPassadoVisible(false);
          ...
          case 'meioComunicItSistema':
            changeTitulo('CITAÇÃO ELETRÔNICA'); // nct
            makeARVisible(false);
            makeSelectorOJVisible(false);
            if(j2.env.modId.modAut.versao.match(/3.2|3.3/)){ //nct
              placeDestinatarioControl('sup');
      ...
      containers : { 
        exp : {
          ...
          itemList : function (){ return mod.exp.gE('SDLinkedElementcitacaoItens');},
          OJ : function (){ return mod.exp.gE('oficialJustica'); },
          valorExecucao : {
            core : function(){ return mod.exp.gE('singleBody.citacao.valorExecucao');},
            br : function(){ return mod.exp.gE('singleBody.citacao.valorExecucao_');} 
          }
        },
        edt : {
          meio : function (){ return mod.edt.gE('selectorcitacaoSelectMeio'); },
          deOrdem : function (){ return mod.edt.gE('selectorDeOrdemDoExpediente'); },
          itemSelector : function (){ return mod.edt.gE('selectorcitacaoItens'); },
          itemsMonitor : function (){ return mod.edt.gE('selectorMonitorcitacaoItens'); },
      ...
      setMonetaryListeners : function(){        
        forEach(pkg.Citacao.bindMonetary.items, function(i){
          evBus.on('onAdd.'+ pkg.Citacao.bindMonetary.selector +'.'+ i, function(ev, arg){
            pkg.Citacao.resolveCtrlMonetario(arg, 'intmItCitacaoExecucaoJec');
          });
          evBus.on('onDel.'+ pkg.Citacao.bindMonetary.selector+'.'+ i, function(ev, arg){
            pkg.Citacao.resolveCtrlMonetario(arg, 'intmItCitacaoExecucaoJec');
          });
        });
        
        evBus.on('afterCreateMonetarioControls.intmItCitacaoExecucaoJec', function(){
        ...
        if(args.monitr){
          forEach(args.monitr.options, function(opt){
            //forEach(pkg.Certidao.bindMonetary.items, function(bIt){
              if(opt.id.itemId() === 'intmItCitacaoExecucaoJec' || opt.id.itemId() === 'intmItCitacaoExecucaoExTiExAudJec')
                if(mod.edt.gE('ctrlMonetario'+item)){
                  ...
                  /* create container */
                  if(pkg.monetario){
                    pkg.monetario.createInputSetMonetario('ctrlMonetario'+item, 'Execucao R$', pkg.Citacao.bindMonetary.container, mod.exp.gE('valorExecucao.'+item), item, j2.env.PJeVars.processo.valorFloat.toFixed(2));
                    return;
                  }
            //});
          });
