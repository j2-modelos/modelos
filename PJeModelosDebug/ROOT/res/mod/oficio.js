...
try {
  (function () { /* inits autoexec */
    ...
    pkg.Oficio = {
      ...
      setEvents : function(){
        evBus.on('DocEditorCore.TabCreated', function(event, edtContainer){
          
          var context = {
            edt : edtContainer.get(0),
            edtCore : function(){
              return edtContainer.get(0);
            },
            exp : mod.exp.gE('j2Exp')
          };

          pkg.AddtionalControls.append(context, 'Oficio.AnotherPage', j2.env.modId.modAut.versao, {resizeOnFinish : false} );

        });
        
        if(pkg.DocEditorCore.readyFired){
          evBus.fire('DocEditorCore.TabCreatedDeffered');
        }
      },
      bindMonetary : {
        items :[
