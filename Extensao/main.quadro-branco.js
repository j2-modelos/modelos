...
function init() {
  ...
  async function _obterEtiqueta(idProcesso){
    return new Promise((resolve, reject)=>{
      const request = {
        ...
        arguments: {
          url: `https://pje.tjma.jus.br/pje/seam/resource/rest/pje-legacy/painelUsuario/processoTags/listar/${idProcesso}`,
          metodo: 'GET',
          conversor: 'json'
        },
        j2Action : 'fetch-via-background',
        comment : "Requisitar fetch ao endpoint de obter etiquetas do processo"
      };

      __sendMessageToServiceWorder(request, async function(messageResp, loadStatus, load){
        const { resposta, erro } = messageResp
        if(erro)
          reject(erro)
  ...
  async function _obterTarefas(idProcesso){
    return new Promise((resolve, reject)=>{
      const request = {
        j2 : true,
        domain : { 
        ...
        arguments: {
          url: `https://pje.tjma.jus.br/pje/seam/resource/rest/pje-legacy/processos/${idProcesso}/tarefas`,
          metodo: 'GET',
          conversor: 'json'
        },
        j2Action : 'fetch-via-background',
        comment : "Requisitar fetch ao endpoint de obter as  tarefas do processo"
      };

      __sendMessageToServiceWorder(request, async function(messageResp){
        const { resposta, erro } = messageResp
        if(erro)
          reject(erro)
