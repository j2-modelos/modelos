...
function evoCalendarFactory(){
  ;(function(factory) {
              ...
              _.initials = {
                  ...
                  dates: {
                      ...
                      pt: {
                          ...
                          noEventForToday: "Nenhum evento ou prazo calculado para exibir",
                          noEventForThisDay: "Nenhum evento ou prazo calculado para exibir",
      ...
      EvoCalendar.prototype.newDate = function(isoOrFormated) {
        var _ = this;

        if(typeof isoOrFormated === 'string' && isoOrFormated.indexOf('/') !== -1){
          var format = _.parseFormat(_.options.format);
          var isoDate = [];
          var dateAr = isoOrFormated.split('/');
          isoDate[0] = dateAr[ format.parts.indexOf('yyyy') !== -1 ? format.parts.indexOf('yyyy') : format.parts.indexOf('yy')];
          isoDate[1] = dateAr[ format.parts.indexOf('m') !== -1 ? format.parts.indexOf('m') : format.parts.indexOf('M') !== -1  ? format.parts.indexOf('M') : format.parts.indexOf('MM') !== -1 ? format.parts.indexOf('MM') : format.parts.indexOf('mm')];
          isoDate[2] = dateAr[ format.parts.indexOf('d') !== -1 ? format.parts.indexOf('d') : format.parts.indexOf('D') !== -1  ? format.parts.indexOf('D') : format.parts.indexOf('DD') !== -1 ? format.parts.indexOf('DD') : format.parts.indexOf('dd')];

          return _.newDate(isoDate.join('-'));
        }else if (Object.prototype.toString.call(isoOrFormated) === '[object Date]')
          return _.newDate(isoOrFormated.toISOString().split('T')[0]); 
        else
          return new Date().newIsoAdjust(isoOrFormated);
      };
      ...
      EvoCalendar.prototype.init = function(init) {
          ...
          if (!$(_.$elements.calendarEl).hasClass('calendar-initialized')) {
              ...
              $(_.$elements.calendarEl).trigger("evoCalendarInit", [_]);
          }
      };
      ...
      EvoCalendar.prototype.isBetweenDates = function(active_date, dates) {
          var sd, ed, ac_dt, _;
          _ = this;
          ac_dt = this.newDate(active_date);
          
          if (dates instanceof Array) {
              sd = _.newDate(dates[0]);
              ed = _.newDate(dates[dates.length - 1]);
          } else {
              sd = _.newDate(dates);
              ed = _.newDate(dates);
          }
          if (sd <= ac_dt && ed >= ac_dt) {
              return true;
          }
          return false;
      };
      ...
      EvoCalendar.prototype.addEventList = function(event_data) {
          ...
          markup.container = $('<div class="event-container" role="button" data-event-index="'+(event_data.id)+'">');
          
          markup.eventIcon = $('<div class="event-icon">')
                  .appendTo(markup.container);
          
          markup.eventBullert = $('<div class="event-bullet-'+event_data.type+'"' 
                  + (event_data.color ? ' style="background-color:'+event_data.color+'"' : '') + '>'
                  ).appendTo(markup.eventIcon);
  
          markup.eventInfo = $('<div class="event-info">')
                  .appendTo(markup.container);
          
          markup.eventInfoText = $('<p class="event-title">')
                  .append( $('<p-text>').text( event_data.name.formatDate ?  _.formatDate(_.newDate(event_data.name.text), _.options.format) : event_data.name /*_.limitTitle(event_data.name); */  ) )
                  .appendTo(markup.eventInfo);
          
          
          if (event_data.badge){ 
            markup.eventInfoBadge = $('<span>')
                    .appendTo(markup.eventInfoText);
            
            if(event_data.badge.jQ)
              markup.eventInfoBadge.append(event_data.badge.jQ);
            else if(event_data.badge.html)
              markup.eventInfoBadge.html(event_data.badge.html);
            else
              markup.eventInfoBadge.text(event_data.badge);
          }
          ...
          event_data.callback && event_data.callback( markup, function (text) {
            text = _.newDate(text);
            return _.formatDate(_.newDate(text), _.options.format);
          });
      ...
      ****************/
      ...
      EvoCalendar.prototype.selectDate = function(event) {
          ...
          var date, year, month, activeDayEl, isSameDate, jsDate;

          if (typeof event === 'string' || typeof event === 'number' || event instanceof Date) {
              jsDate = new Date(event);
              ...
              jsDate = _.newDate(date);
          }
          isSameDate = _.$active.date === date;
          // Set new active date
          _.$active.date = date;
          _.$active.event_date = date;
          _.$active.jsDate = jsDate;
          // Remove active class to all
      ...
      EvoCalendar.prototype.addCalendarEvent = function(arr) {
          ...
          function addEvent(data) {
              ...
              if((_.options.calendarEvents||[]).filter(function(el, dix, arr){
              ...
              if (data.date instanceof Array) {
                  for (var j=0; j < data.date.length; j++) {
                      if(isDateValid(data.date[j])) {
                          data.date[j] = _.formatDate(_.newDate(data.date[j]), _.options.format);
                      }
                  }
              } else {
                  if(isDateValid(data.date)) {
                      data.date = _.formatDate(_.newDate(data.date), _.options.format);
                  }
              }
...
function checkEvoCalendarFactory() { // tappac as new
  if (typeof jQ3 !== 'undefined') {
    ...
    setTimeout( checkEvoCalendarFactory, 10 );
  }
}
checkEvoCalendarFactory();