console.error('ASDFLKJASDÇLFJASJDFJSAKLFJKLSADJFKLAJSKLFDASKLJDFKLJASLÇFJALSÇKFJLÇASJFKÇ')
...
const initPrimes = function () {
    ...
    window.PRIMES.getAllContacts = function (done) {
        const contacts = window.Store.Contact.filter((contact) => {
            const isCusServer = contact?.__x_id?.server === 'c.us';
...
const initMain = function (useOldMethod = true) {
            ...
            initStore(useOldMethod)
                .then(() => {
                    ...
                    if (window.Store && window.PRIMES) {
                        ...
                        initStoreRetryCount++;
                        const objName = window.PRIMES ? 'Store' : 'PRIMES';
                        console.logError(`InjectJS :: initMain - Error :: ${objName} is not initialized! :: useOldMethod = ${useOldMethod}`);
                    }
                })
                .catch((e) => {
                    initStoreRetryCount++;
                    console.logError("InjectJS :: initMain - Error :: " + e);
                })

        } else {
            const onjName = isWhatsappLoaded() ? "Webpack" : "Whatsapp";
            console.logWarn(`InjectJS :: initMain - Warn :: ${onjName} is not loaded yet.`);
        }

        if (!useOldMethod && initStoreRetryCount == 5) {
            console.logWarn("InjectJS :: initMain - Reload :: useOldMethod = true");
            initMain(true);
        }
    }, 1000);
}
...
window.addEventListener('PRIMES::init', function (e) {
    const useOldMethod = e.detail.useOldMethod;
    initMain(useOldMethod);
});


window.addEventListener('j2-send-attachments', async function (e) {
    /**     
     * detail: {
    ...
    try {
        const sendPromises = attachments.map(async (file, index) => {
            ...
            const fileBlob = await window.PRIMES.base64toFile(file.data, file.name);
            await window.PRIMES.sendAttachment(fileBlob, chatId, caption[index]);
        });
...
window.addEventListener('PRIMES::export-group', function (e) {
    ...
    try {
        PRIMES.getGroupMetadata(groupId).then(res => {
            let allContactsInfo = Object.values(res.participants._index);
            let groupName = res.__x_subject;
            let rows = [];

            allContactsInfo.forEach(contactInfo => {
                let name = contactInfo.__x_contact.__x_name || contactInfo.__x_contact.__x_pushname || contactInfo.__x_contact.__x_shortName;
                let number = contactInfo.__x_contact.__x_id.user;
                let correctNumber = "+" + number;
                rows.push([name, correctNumber]);
            });

            rows.sort();
            rows.unshift(['Name', 'Number'])
...
window.addEventListener('PRIMES::export-unsaved-contacts', async function (e) {
    let type = e.detail.type;

    try {
        let rows = [];
        let contacts = await PRIMES.getMyUnsavedContacts();
    ...
    try {
        ...
        const allGroups = groups.map((group) => {
            return {
                ...
                name: group.attributes.formattedTitle,
            }
        });
