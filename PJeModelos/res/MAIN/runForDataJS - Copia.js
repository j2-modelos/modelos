...
(function () {
  ...
  window.j2.mod._._opW = {
     center : function(url, name, idProcesso, winSize, scrolled, callback){ // wa
        if(name){
          if(idProcesso)
            name+=idProcesso;
        }
      ...
      corner : function(url, name, idProcesso, winSize, scrolled, callback){ // pdf as new
        if(name){
          if(idProcesso)
            name+=idProcesso;
        }
...
try {
  ...
  (function () {     
    ...
    evBus.once('afterLoadRunLibs', function(event, definitiions){
      ...
      if(j2.api && j2.api.extDependency.length){ // pl as new
