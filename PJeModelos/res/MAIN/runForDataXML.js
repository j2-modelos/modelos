/*
 * 
 * Arquivo RUN para fonte de dados baseados em XML
 */
...
(function(){
  ...
  j2.env.j2U.log = function(txt){
    ...
    pp.innerHTML = txt;
    window.document.getElementById('upLog').appendChild(pp);
    //jQ(window.document.body).find('#upLog')[0].append(jQ('<p/>', {text : txt, class : 'pLog'})[0]);
  };
  j2.env.j2U.progBar = function(val){
    jQ(window.document.getElementById('uBar')).css('width', val +'%');
    jQ(window.document.getElementById('uBar')).text(val*1+'%');
  };
  ...
  jQ(css).appendTo(window.document.body);
  jQ(css2).appendTo(window.document.body);
  jQ(html).appendTo(window.document.body);

  j2.env.j2U.log('Carregando Modelos j2');
  j2.env.j2U.log('dados: XML');
...
window.j2.mod._._parseVar = function (var_) {
  var v = var_.split('#{')[1];
  v = v.split('}')[0];
  v = v.split('.');
  var _='', r;
...
try {
  ...
  (function () {     
    ...
    window.j2.env.modId = {
      'id': document.getElementById('idModelo').value
    };
    window.j2.env.xmls = { };
    window.j2.env.css = { };
    ...
    window.j2.res.MAIN.artfacts = {
      'ref': function () { return (window.j2ModDebug)?(window.j2.ut.isHttps())? 'exkbd8ly9xgnw6s' : '/res/MAIN/artfacts.js' : 'nnll2mref9pfcf1'; },                
    ...
    window.j2.modelo.exp.gE('modeloBooterBody').remove();
    ...
    evBus.once('loaded-ROOT/res/MAIN/artfacts.js', function(){    
      ...
      evBus.once('loaded-'+window.j2.res.XML.unidadesAutorizadas.lib, function(event, xml){
        ...
        window.j2.env.xmls.unidadesAutoriadas = xml;
        j2.env.j2U.progBar(35);
        j2.env.j2U.log('unidadesAutorizadas.xml');
      ...
      evBus.once('loaded-'+window.j2.res.XML.modelos.lib, function(event, xml){
        ...
        window.j2.env.xmls.modelos = xml;
        j2.env.j2U.progBar(40);
        j2.env.j2U.log('modelos.xml');
      ...
      evBus.once('loaded-'+window.j2.res.XML.baseClasses.lib, function(event, xml){
        ...
        window.j2.env.xmls.baseClasses = xml;
        j2.env.j2U.progBar(45);
        j2.env.j2U.log('baseClasses.xml');
      ...
      evBus.once('loaded-'+window.j2.res.XML.usuarios.lib, function(event, xml){
        ...
        window.j2.env.xmls.usuarios = xml;
    ...
    function crateAutoResize(){
      evBus.on('Editor.onResize', function(event){
        var _ = j2.modelo.edt.win;
        _.resizeTo(_.outerWidth, j2.modelo.edt.gE('DocEditorCore').clientHeight+110);
      });
    };    
    ...
    evBus.once('builder.afterBuildModSet.main', 10000, function(event, definitiions){
      ...
      var _ = {
        //butId : 'taskInstanceForm:minutaEmElaboracao-' + tsk + ':minutaEmElaboracao-' + tsk + 'Decoration2:minutaEmElaboracao-' + tsk + 'BotaoSalvar'
        //butIdSec : 'taskInstanceForm:minutaInfoseg-' + tsk + ':minutaInfoseg-' + tsk + 'Decoration2:minutaInfoseg-' + tsk + 'BotaoSalvar',
        but : j2.modelo.par.doc.querySelector('input.btn.btn-primary[value="Salvar"]')
      };
      console.log('################################## auto save doc on editor');
      //if(_.but && _.but.id !== 'taskInstanceForm:update' && _.but.id !== _.butId && _.but.id !== _.butIdSec)
      if(j2.modelo.par.win.location.pathname === '/pje/Processo/ConsultaProcesso/Detalhe/listAutosDigitais.seam'  || j2.modelo.par.win.location.pathname === '/pje/Processo/ConsultaProcesso/Detalhe/detalheProcessoVisualizacao.seam')
        _.but.click();
      
      
      j2.modelo.edt.gE('imgLoadBuild').remove();
    ...
    evBus.once('afterLoadRunLibs', function(event, definitiions){
      ...
      i.src = 'https://pje.tjma.jus.br/pje/styleSkinInfox/vermelho/imagens/status/ajax-loader-retangular.gif';
      i.id='imgLoadBuild';
    ...
    evBus.on('onPreBuildWindowEdition', 10000, function(event){
      ...
      myWindow = j2.mod._._opW.center( _.url, _.name, _.idProcesso, _.winSize, _.scrolled );
