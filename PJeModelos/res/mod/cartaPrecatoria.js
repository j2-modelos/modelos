            ...
            /*forEach(pkg.Mandado.bindMonetary.items, function(bIt){*/
              if(opt.id.itemId() === item)
                if(mod.edt.gE('ctrlMonetario'+item)){
                  ...
                  if(pkg.monetario){
                    pkg.monetario.createInputSetMonetario('ctrlMonetario'+item, 'Execucao R$', pkg.CartaPrecatoria.bindMonetary.container, mod.exp.gE('valorExecucao.intmItCitacaoExecucaoJec'), item, j2.env.PJeVars.processo.valorFloat.toFixed(2));
                    return;
                  }
          /*  });*/
          });
