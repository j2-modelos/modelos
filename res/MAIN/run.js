...
var _windowDocument =
  j2.api && j2.api.winExt ? j2.api.winExt.document : window.document //mc
...
;(function () {
  ...
  var css2 =
    '<style id="uS2">#uBar {border-radius: 5px; width: 1%; height: 15px; background-color: #448aff;; text-align: center;  line-height: 15px;   color: white;     font-size: 10px; font-family:"Segoe UI", Tahoma, Geneva, Verdana, sans-serif;} #uProg { border-radius: 5px; width: 20%; margin:auto; background-color: #bbd0f3;; }</style>'
  var html =
    '<div id="uDiv"><div class="divCont"><div class="svg-preloader"><svg version="1.1" height="30" width="30" viewBox="0 0 75 75"><circle cx="37.5" cy="37.5" r="33.5" stroke-width="8"></circle></svg></div></div><div id="uProg"><div id="uBar"></div> </div><div id="upLog"></div></div>'
...
;(function () {
  ...
  if (w.removeHTMLForms) {
    w.removeHTMLForms = function () {
      console.log('j2 removed functionality of removeHTMLForms ')
    }
  }
  if (w.removeHTMLComponent) {
    w.removeHTMLComponent = function () {
      console.log('j2 removed functionality of removeHTMLComponent ')
    }
  }
  if (w.removerTagHTML) {
    w.removerTagHTML = function () {
      console.log('j2 removed functionality of removerTagHTML ')
    }
  }
  if (w.removeCaracterEspecial) {
    w.removeCaracterEspecial = function () {
...
;(function () {
  ...
  if (!window.j2) window.j2 = {}
  if (!window.j2.mod) window.j2.mod = {}
  if (!window.j2.mod._) window.j2.mod._ = {}
  ...
  window.j2.mod._._42 = function () {
    ...
    function EventBus() {
      this._listeners = {}
      this._fireLogging = []
      if (!fireLogging) this._fireLogging = 'DISABLED'
    ...
    EventBus.prototype.fire = function (type, data) {
      ...
      if (fireLogging) this._fireLogging.push(_toLog)
  ...
  window.j2.mod._._149 = function () {
    ...
    return function bindCallback(func, thisArg, argCount) {
      if (typeof func != 'function') {
        return identity
      }
  ...
  String.prototype.captFirst = function () {
    //wmg
    var _ar = this.split(' ')
    var r
    for (var i = 0; i < 1; i++)
  ...
  String.prototype.capt = function () {
    ...
    for (var i = 0; i < _ar.length; i++)
      _ar[i] =
        _ar[i].length < 3
          ? _ar[i].toLowerCase()
          : _ar[i].charAt(0).toUpperCase() + _ar[i].slice(1).toLowerCase() // ndlg2
    return _ar.join(' ')
  }
  ...
  String.prototype.trimSpaces = function () {
    ...
    var i = 0
    do {
      if (_[i].length === 0 || _[i] === [10].fromASCII()) _.splice(i, 1)
      else i++
    } while (i < _.length)
  ...
  String.prototype.toURLDropboxV2 = function (nomeArquivoComExtensao) {
    ...
    const resolveDomain = () => {
      let extensao = !nomeArquivoComExtensao
        ? 'pdf'
        : (() => {
            const splited = nomeArquivoComExtensao.split('.')
            if (this.length > 1) return splited.pop()
    ...
    if (typeof idD !== 'undefined' && typeof rlkey !== 'undefined') {
      return `${resolveDomain()}/${idD}/${encodeURI(
        nomeArquivoComExtensao || _guid()
      )}${nomeArquivoComExtensao ? '' : '.pdf'}?rlkey=${rlkey}&raw=1`
    } else return 'https://www.tjma.jus.br'
  }
  ...
  Array.prototype.joinListE = function (applyEachString, sort) {
    // ndlg2
    var isString = new window.j2.mod._._203()
    var isFunction = new window.j2.mod._._198() // pl

    if (typeof sort === 'undefined' || sort === true) this.sort()
    ...
    for (var i = 0; i < this.length; i++) {
      txT = tx.length
        ? (i === this.length - 1 ? ' e ' : ', ') + this[i]
        : this[i]
      tx += applyEachString
        ? isString(applyEachString)
          ? txT[applyEachString]()
          : isFunction(applyEachString)
            ? applyEachString(txT)
            : txT
        : txT
    }
    ...
    while (text.indexOf('#:') !== -1) {
      ...
      for (var i = 1; i < v.length; i++)
        if (_.length) {
          ...
          inTagCount++
          if (v[i].indexOf('}') !== -1)
            if (_.split('{').length === _.split('}').length) break
        } else {
          _ = v[i]
          if (v[i].indexOf('}') !== -1)
            if (_.split('{').length === _.split('}').length) break
      ...
      v = v.split('}')
      _ = []
      for (var i = 0; i <= inTagCount; i++) _[i] = v[i]
  ...
  window.j2.mod._._opW = {
    center: function (
      ...
      if (winSize)
        prm =
          ...
          ', left=' +
          lf
      else prm = 'width=' + w + ', height=' + h + ', top=' + tp + ', left=' + lf

      if (scrolled) {
        if (scrolled === true) prm += ', scrollbars=1'
      } else prm += ', scrollbars=1'
      ...
      setInterval(function () {
        win.document &&
          (win.document.title =
            'j2 ' +
            (flg ? j2.env.PJeVars.processo.numeroCurto : altTitle || name))
        flg = !flg
      }, 1500)
    ...
    corner: function (
      ...
      setInterval(function () {
        win.document.title =
          'j2 ' + (flg ? j2.env.PJeVars.processo.numeroCurto : altTitle || name)
        flg = !flg
      }, 1500)
  ...
  window.j2.mod._._formatarISOStringDataParaDataPorExtenso = function (
    isoDate
  ) {
    const months = [
      'janeiro',
  ...
  if (_windowDocument.getElementById('PJeVarsXML'))
    window.j2.mod._.PJeVars = new DOMParser().parseFromString(
      _windowDocument.getElementById('PJeVarsXML').innerHTML,
      'text/html'
    )
  ...
  else console.error('PJeVarsXML não foi localizado')
})()
...
window.j2.mod._._j2JSON = function () {
  // ndlg2
...
window.j2.mod._._injectCSS = function (css) {
  var css_ = function () {
    return (
      ...
      css.fileName +
      '">' +
      JSON.parse(localStorage.getItem(css.lib)).content +
      '</style>'
    )
  }
...
window.j2.mod._._injectScript = function (scrp, loc) {
  var css_ = function () {
    return (
      ...
      scrp.fileName +
      '">' +
      JSON.parse(localStorage.getItem(scrp.lib)).content +
      '</style>'
    )
  }
...
window.j2.mod._.CookieJ2 = function (bn) {
  ...
  return {
    ...
    set: function (key, value) {
      _cookies.set(_baseCookieName() + '.' + key, value, {
...
window.j2.mod._._cookies = function () {
  // ard as new
  'use strict'
...
window.j2.mod._._getJsonFromUrl = function (url) {
  // ndlg2
...
window.j2.mod._._autoHide = function (mb) {
  // ndlg4
  var _mb = mb
...
window.j2.mod._._del = function (_class) {
  // ndlg6
  var __class = _class
...
window.j2.mod._._inherits = function (el, _, className, superClassName) {
  // tappac as new
  var isArray = new window.j2.mod._._197(),
  ...
  var _getSuperClasse = function (supName, thisObject) {
    if (
      el._ &&
      el._.ih__class__ &&
      el._.ih__class__.super &&
      el._.ih__class__.super[supName]
    ) {
      if (thisObject) {
        _['$' + supName] = el._.ih__class__.super[supName]._this
        return el._.ih__class__.super[supName]._this
      } else return el._.ih__class__.super[supName]
    } else return '[NO SUPER CLASSE]'
  }
 ...
 */
window.j2.mod._._Lockr = function (prefix, storage) {
  // tappac as new
  'use strict'

  var Lockr = {}
  if (!storage) storage = window.localStorage
  else storage = window[storage]
      ...
      if (console)
        console.warn(
          "Lockr didn't successfully save the '{" +
            ...
            ': ' +
            value +
            "}' pair, because the storage is full."
        )
    }
  }
  ...
  Lockr.get = function (key, missing, options) {
    ...
    if (!value) {
      ...
      if (typeof value.expiration !== 'undefined') {
        var now = new Date().getTime()
        if (now > value.expiration) return missing
      }
      return value.data
    }
  }
  ...
  Lockr.sadd = function (key, value, options) {
    ...
    for (var i = 0; i < values.length; i++) {
      if (JSON.stringify(values[i]) === JSON.stringify(value)) return
    }
      ...
      if (console)
        console.warn(
          "Lockr didn't successfully add the " +
            ...
            ' to ' +
            key +
            ' set, because the storage is full.'
        )
    }
  }
...
window.j2.mod._._convertBoolean = function (value) {
  ...
  throw new Error(
    'Valor não pode ser boolean. Apenas boolean, number ou string podem ser avaliados'
  )
}
...
try {
  ...
  ;(function () {
      ...
      var libCheck = function (libs) {
        ...
        if (isArray(libs)) {
          ...
          forEach(libs, function (e) {
            if (isString(e)) areOk &= isObject(window.j2.modelo.exp.gE(e))
            else if (isObject(e)) {
              if (e.lib) areOk &= isObject(window.j2.modelo.exp.gE(e.lib))
              else {
                areOk &= false
                console.error(
                  'LibCheck: lib type invalid. String or Objet res.'
                )
              }
            } else {
              areOk &= false
        ...
        if (isString(libs)) return isObject(window.j2.modelo.exp.gE(libs))
    ...
    window.j2.env.modId = {
      id: _windowDocument.getElementById('idModelo')
        ? _windowDocument.getElementById('idModelo').value
        : j2.api.idModelo, //mc
    }
    window.j2.env.xmls = {}
    window.j2.env.css = {}
    ...
    window.j2.modelo = {
      ...
      par: {
        /* parente: task or process */ win: _window.parent,
    ...
    if (window.j2.modelo.exp.gE('modeloBooterBody'))
      //mc
    ...
    evBus.once('loaded-ROOT/res/MAIN/artfacts.js', function () {
      ...
      ;(_ap._progresCount = 15), (_ap.isMainBoot = true)
      ...
      evBus.once(
        'loaded-' + window.j2.res.XML.unidadesAutorizadas.lib,
        function (event, xml) {
          console.log(
            'fired once loaded-' + window.j2.res.XML.unidadesAutorizadas.lib
          )
          window.j2.env.xmls.unidadesAutoriadas = window.j2.mod._._xmlDecode(
            j2.res.XML.unidadesAutorizadas.xmlEncode.load
          )
      ...
      evBus.once(
        'loaded-' + window.j2.res.XML.modelos.lib,
        function (event, xml) {
          console.log('fired once loaded-' + window.j2.res.XML.modelos.lib)
          window.j2.env.xmls.modelos = window.j2.mod._._xmlDecode(
            j2.res.XML.modelos.xmlEncode.load
          )
      ...
      evBus.once(
        'loaded-' + window.j2.res.XML.baseClasses.lib,
        function (event, xml) {
          console.log('fired once loaded-' + window.j2.res.XML.baseClasses.lib)
          window.j2.env.xmls.baseClasses = window.j2.mod._._xmlDecode(
            j2.res.XML.baseClasses.xmlEncode.load
          )
      ...
      evBus.once(
        'loaded-' + window.j2.res.XML.usuarios.lib,
        function (event, xml) {
          console.log('fired once loaded-' + window.j2.res.XML.usuarios.lib)
          window.j2.env.xmls.usuarios = window.j2.mod._._xmlDecode(
            j2.res.XML.usuarios.xmlEncode.load
          )
      ...
      evBus.once(
        'loaded-' + window.j2.res.XML.classStyles.lib,
        function (event, xml) {
          console.log('fired once loaded-' + window.j2.res.XML.classStyles.lib)
          window.j2.env.xmls.classStyles = xml
          window.j2.env.xmls.classStyles = window.j2.mod._._xmlDecode(
            j2.res.XML.classStyles.xmlEncode.load
          )
      ...
      evBus.once(
        'loaded-' + window.j2.res.lib.jquery3.lib,
        function (event, css) {
    ...
    evBus.once('afterLoadRunLibs___________', function () {
      // boot
      var templateXML =
        '<?xml version="1.0" encoding="utf-8"?><Definitions id="UnidadesAutorizados"/>'

      try {
        if (window.j2 && window.j2.mod && window.j2.mod.j2Moddle) {
          console.log('j2Moddle foi construído')

          window.j2.mod.j2Moddle.fromXML(
            templateXML,
            'j2:Definitions',
            function (err, definitions, context) {
              if (definitions) console.log('Teste de parse XML OK.')
        ...
        } else console.error('j2Moddle não foi construído')
      } catch (e) {
    ...
    evBus.once('j2Moddle.loaded', function (event, definitiions) {
      ...
      window.j2.mod.j2Moddle.fromXML(
        window.j2.env.xmls.unidadesAutoriadas,
        'j2:Definitions',
        function (err, definitions, context) {
          if (definitions) window.j2.env.xmls.unidadesAutoriadas = definitions
    ...
    evBus.once('parsed.unidadesAutorizadas', function (event, definitiions) {
      ...
      window.j2.mod.j2Moddle.fromXML(
        window.j2.env.xmls.modelos,
        'j2:Definitions',
        function (err, definitions, context) {
          if (definitions) window.j2.env.xmls.modelos = definitions

          if (err) console.error('err get defintions from XML - modelos')
    ...
    evBus.once('parsed.modelos', function (event, definitiions) {
      ...
      window.j2.mod.j2Moddle.fromXML(
        window.j2.env.xmls.usuarios,
        'j2:Definitions',
        function (err, definitions, context) {
          if (definitions) window.j2.env.xmls.usuarios = definitions

          if (err) console.error('err get defintions from XML - usuarios')
    ...
    evBus.once('parsed.usuarios', function (event, definitiions) {
      ...
      window.j2.mod.j2Moddle.fromXML(
        window.j2.env.xmls.baseClasses,
        'j2:Definitions',
        function (err, definitions, context) {
          if (definitions) window.j2.env.xmls.baseClasses = definitions

          if (err) console.error('err get defintions from XML - baseClasses')
    ...
    evBus.once('parsed.classStyles', function (event, definitiions) {
      ...
      window.j2.mod.j2Moddle.fromXML(
        window.j2.env.xmls.classStyles,
        'j2:Definitions',
        function (err, definitions, context) {
          if (definitions) window.j2.env.xmls.classStyles = definitions

          if (err) console.error('err get defintions from XML - classStyles')
    ...
    function registerRevisionAndUpdate() {
      evBus.once(
        'loaded-' + window.j2.res.XML.update.lib,
        function (event, xml) {
          console.log('fired once loaded-' + window.j2.res.XML.update.lib)
          window.j2.env.xmls.update = xml

          window.j2.mod.j2Moddle.fromXML(
            window.j2.env.xmls.update,
            'j2:Definitions',
            function (err, definitions, context) {
              if (definitions) window.j2.env.xmls.update = definitions
              ...
              var upXML = {
                update: {
                  forceUpdateNextLoad:
                    window.j2.env.xmls.update.update.forceUpdateNextLoad,
                  currentRevision:
                    window.j2.env.xmls.update.update.currentRevision,
                },
              }
    ...
    function crateAutoResize() {
      evBus.on('Editor.onResize', function (event, j2Win) {
        ...
        if (_j2Win.gE('DocEditorCore') || _j2Win.gE('WS'))
          // wa
          _j2Win.win.resizeTo(
            _j2Win.win.outerWidth,
            (_j2Win.gE('DocEditorCore') || _j2Win.gE('WS')).clientHeight + 110
          )
      })
    }

    evBus.once(
      'builder.afterBuildModSet.main',
      10000,
      function (event, definitiions) {
        ...
        if (j2.mod.clsCnstr.AutoSave) evBus.fire('AutoSave.recover.initialize')

        var sz = j2.mod.clsCnstr.DocEditorCore.size
        if (sz) j2.modelo.edt.win.resizeTo(sz.width, sz.height)
        ...
        if (j2?.api?.idModelo)
          evBus.fire(
            `builder.afterBuildModSet.externalApi.${j2.api.idModelo}`,
            definitiions
          )
      }
    )
    ...
    evBus.once('afterLoadRunLibs', function (event, definitiions) {
      ...
      if (
        j2.api &&
        j2.api.extDependency.length &&
        j2.api.isMainBoot !== undefined &&
        !j2.api.isMainBoot
      ) {
        // pl as new // boot
    ...
    function processJ2ApiBoot() {
      ...
      forEach(j2.api.extDependency, function (extD) {
        ...
        evBus.once('loaded-' + _lib.lib, function (event, xml) {
          ...
          if (
            (_lib.type === 'j2/javascript' || _lib.type === 'j2/styleSheet') &&
            _lib.injectWins
          ) {
            // hds as new // chosen
          ...
          if (!xml) xml = xml = window.j2.mod._._xmlDecode(_lib.xmlEncode.load)
          ...
          function _parse(_xml, _extD, __lib) {
            if (
              !(window.j2 && window.j2.mod && window.j2.mod.j2Moddle) &&
              !___J2_API_EXT_DEPENDENCY['"' + _extD.lib + '"'].processed
            ) {
              setTimeout(function () {
    ...
    function processJ2ApiExtDependecy(
      dependencies,
      eventId,
      contexto,
      callback
      ...
      forEach(dependencies || j2.api.extDependency, function (extD) {
        ...
        evBus.once('loaded-' + _lib.lib, function (event, xml) {
          ...
          if (!_lib.xmlEncode) {
            ___J2_API_EXT_DEPENDENCY['"' + extD.lib + '"'].processed = true
            if (contexto?.xmls) contexto.xmls[last(extD.lib.split('.'))] = xml
            evBus.fire(
              'j2ApiExtDependency.onLoadPartialContent' +
                (eventId ? eventId : '')
            )
            return
          }

          if (!xml) xml = window.j2.mod._._xmlDecode(_lib.xmlEncode.load)
      ...
      evBus.on(
        'j2ApiExtDependency.onLoadPartialContent' + (eventId ? eventId : ''),
        function (e) {
          var _ctrl = true
          forEach(___J2_API_EXT_DEPENDENCY, function (dep) {
            _ctrl = _ctrl && dep.processed
          ...
          if (_ctrl)
            if (callback)
              defer(() => callback()) // padrão
            else libLoad(window.j2.res.MAIN.builder)
        }
      )
    }
    ...
    evBus.on('onPreBuildWindowEdition', 10000, function (event) {
      ...
      else myWindow = j2.api.winEdt

      myWindow.j2 = window.j2
      window.parent.j2 = window.j2

      window.j2.modelo.edt = {
        /* Editor app */ win: myWindow,
        doc: myWindow.document,
        gE: function (id) {
          return myWindow.document.getElementById(id)
        },
...
if (j2)
  j2.j2T = {
    ...
    addContatc: function () {
      $.post(
        'https://content-people.googleapis.com/v1/people:createContact?alt=json',
        {
          names: [{ givenName: '006.418.053-06 - ELMO DE OLIVEIRA DE MORAES' }],
          phoneNumbers: [
            { type: 'PJe WhatsApp Contact', value: '(99)98136-4744' },
          ],
        },
        function (result) {
          console.log(result)
    ...
    addScript: function () {
      $.getScript('https://accounts.google.com/gsi/client', function () {
        console.log('foi gooooooooooooooooooooooooogle')
      })
    },
    writeButton: function () {
      $('body').append(
        $('<div>', {
          id: 'g_id_onload',
          'data-client_id':
            '834670749432-geq3lufrgfjl0u8i9mhrmih8htfuueuh.apps.googleusercontent.com',
          'data-context': 'use',
          'data-ux_mode': 'popup',
